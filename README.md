# Snapchat Rejected Headline Monitor — Public-Safe GitHub Setup

This is the same multi-Ad-Squad headline bot, packaged for a **new public GitHub
repository**. It keeps credentials and target identifiers in GitHub Secrets, redacts
private routing information from workflow logs, and commits only encrypted retry state.
...
4. Upload everything **inside** the extracted `snapchat-headline-public-safe` folder.
...
All Snapchat credentials must belong to the same OAuth app. The authenticated Snapchat
user must have permission to the configured Ad Account.
...
Copy the one generated value into the GitHub Secret `STATE_ENCRYPTION_KEY`. Keep that key
unchanged. If it is deleted or replaced, the bot cannot decrypt its saved history.
...
Open **Actions → Snapchat Multi-Squad Headline Monitor → Run workflow** and select:
